...
# Takes ~XXX hrs with 108 combos with N=2^12 and 50 threads:
t0 <- Sys.time()
sobol_sims <- map(1:length(sobol_mats), one_struct_sobol_sims,
                .progress = .prog_args)
write_rds(sobol_sims, "sobol-sims.rds", compress = "gz")
t1 <- Sys.time()
